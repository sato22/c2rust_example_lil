 ...
 */
    #[no_mangle]
    fn lil_error(lil: lil_t, msg: *mut *const libc::c_char, pos: *mut size_t)
     -> libc::c_int;
    #[no_mangle]
    fn lil_new() -> lil_t;
    #[no_mangle]
    ...
    #[no_mangle]
    fn lil_register(lil: lil_t, name: *const libc::c_char,
                    proc_0: lil_func_proc_t) -> libc::c_int;
    #[no_mangle]
    fn lil_parse(lil: lil_t, code: *const libc::c_char, codelen: size_t,
                 funclevel: libc::c_int) -> lil_value_t;
    #[no_mangle]
    fn lil_callback(lil: lil_t, cb: libc::c_int, proc_0: lil_callback_proc_t);
    #[no_mangle]
    fn lil_list_to_value(list: lil_list_t, do_escape: libc::c_int)
     -> lil_value_t;
    #[no_mangle]
    ...
    #[no_mangle]
    fn lil_set_var(lil: lil_t, name: *const libc::c_char, val: lil_value_t,
                   local: libc::c_int) -> lil_var_t;
}
 ...
 -> *mut libc::c_char {
    ...
    while i < argc {
        ...
        if i != 0 as libc::c_int as libc::c_ulong {
            cmd =
                realloc(cmd as *mut libc::c_void,
                        (cmdlen + 1 as libc::c_int) as libc::c_ulong) as
                    *mut libc::c_char;
            let fresh0 = cmdlen;
            cmdlen = cmdlen + 1;
        ...
        cmd =
            realloc(cmd as *mut libc::c_void,
                    (cmdlen as libc::c_ulong).wrapping_add(len)) as
                *mut libc::c_char;
        memcpy(cmd.offset(cmdlen as isize) as *mut libc::c_void,
               *argv.offset(i as isize) as *const libc::c_void, len);
        cmdlen =
            (cmdlen as libc::c_ulong).wrapping_add(len) as libc::c_int as
                libc::c_int;
        i = i.wrapping_add(1)
    }
    cmd =
    ...
    *cmd.offset(cmdlen as isize) = 0 as libc::c_int as libc::c_char;
    p = popen(cmd, b"r\x00" as *const u8 as *const libc::c_char);
    ...
    if !p.is_null() {
        ...
        loop  {
            bytes =
                fread(buff.as_mut_ptr() as *mut libc::c_void,
                      1 as libc::c_int as size_t,
                      1024 as libc::c_int as size_t, p) as ssize_t;
            if !(bytes != 0) { break ; }
            retval =
                realloc(retval as *mut libc::c_void,
                        size.wrapping_add(bytes as libc::c_ulong)) as
                    *mut libc::c_char;
            memcpy(retval.offset(size as isize) as *mut libc::c_void,
                   buff.as_mut_ptr() as *const libc::c_void,
                   bytes as libc::c_ulong);
            size =
                (size as libc::c_ulong).wrapping_add(bytes as libc::c_ulong)
                    as size_t as size_t
        }
        retval =
            realloc(retval as *mut libc::c_void,
...
unsafe extern "C" fn fnc_readline(mut lil: lil_t, mut argc: size_t,
    ...
    loop  {
        ...
        if ch as libc::c_int == -(1 as libc::c_int) { break ; }
        if ch as libc::c_int == '\r' as i32 { continue ; }
        if ch as libc::c_int == '\n' as i32 { break ; }
        if len < size {
            size =
                (size as
                    ...
                    as size_t;
            buffer =
                realloc(buffer as *mut libc::c_void, size) as
                    *mut libc::c_char
        }
        let fresh3 = len;
...
unsafe extern "C" fn repl() -> libc::c_int {
    ...
    lil_register(lil, b"writechar\x00" as *const u8 as *const libc::c_char,
                 Some(fnc_writechar as
                          unsafe extern "C" fn(_: lil_t, _: size_t,
                                               _: *mut lil_value_t)
                              -> lil_value_t));
    lil_register(lil, b"system\x00" as *const u8 as *const libc::c_char,
                 Some(fnc_system as
                          unsafe extern "C" fn(_: lil_t, _: size_t,
                                               _: *mut lil_value_t)
                              -> lil_value_t));
    lil_register(lil, b"readline\x00" as *const u8 as *const libc::c_char,
    ...
    lil_callback(lil, 0 as libc::c_int,
    ...
    while running != 0 {
        ...
        result =
            lil_parse(lil, buffer.as_mut_ptr(), 0 as libc::c_int as size_t,
                      0 as libc::c_int);
        strres = lil_to_string(result);
        if *strres.offset(0 as libc::c_int as isize) != 0 {
            printf(b"%s\n\x00" as *const u8 as *const libc::c_char, strres);
        }
        lil_free_value(result);
        if lil_error(lil, &mut err_msg, &mut pos) != 0 {
            printf(b"error at %i: %s\n\x00" as *const u8 as
                       *const libc::c_char, pos as libc::c_int, err_msg);
        }
    }
    lil_free(lil);
    return exit_code;
}
unsafe extern "C" fn nonint(mut argc: libc::c_int,
 ...
 -> libc::c_int {
    ...
    lil_set_var(lil, b"argv\x00" as *const u8 as *const libc::c_char, args,
                0 as libc::c_int);
    lil_free_value(args);
    tmpcode =
        malloc(strlen(filename).wrapping_add(256 as libc::c_int as
                                                 libc::c_ulong)) as
            *mut libc::c_char;
    sprintf(tmpcode,
    ...
    result =
        lil_parse(lil, tmpcode, 0 as libc::c_int as size_t, 1 as libc::c_int);
